...
from core_modules.scheduling.scheduling_helper import get_next_week_for_date, get_next_day_for_date, \
    get_next_year_for_date, get_next_month_for_date, get_next_full_hour
...
class CalendarInteractor(EventReceiver):
    ...
    async def handle_specific_event(self, event: BaseEvent):
        if isinstance(event, CalendarRefreshEvent):
            ...
            next_events = self.get_next_events(await self.fetch_calendar_events(), limit=event.limit)
            ...
            today_events = self.get_events_for_today(await self.fetch_calendar_events(), limit=event.limit)
    ...
    async def fetch_and_schedule_refresh_task(self):
        ...
        next_events = self.get_next_events(events, limit=1)
    ...
    async def schedule_auto_refresh_task(self):
        """
        Schedules an event on startup that runs hourly to trigger the refresh task in case new events have been added
        in the last hour.
        :return:
        """
        await self.put_event(ScheduleEventExecutionEvent(get_next_full_hour(),
                                                         CalendarRefreshEvent(),
    ...
    def _get_next_valid_event(original_event_schedule: CalendarEventSchedule, specific_event: SpecificCalendarEvent,
        ...
        while specific_event.start < datetime.now(
                tzlocal.get_localzone()) or specific_event in already_scheduled_events_list:
    ...
    def get_next_events(self, events, limit=3):
        next_events = []
